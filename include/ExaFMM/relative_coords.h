...
class array {
 ...
 public:
  ...
  constexpr const ElemT& operator[](size_t i) const noexcept {
    return m_data[i];
  }
 ...
 * placed on 3D grid-points from
 *[-includeBoxRadius:pointStride:includeBoxRadius] in each dimension, except
 *where they lie within (not <=, <) excludeBoxRadius.
 **/
...
constexpr auto create_coords_array_backward() {
  ...
  for (int k = -incR; k <= incR; k += stride) {
    for (int j = -incR; j <= incR; j += stride) {
      for (int i = -incR; i <= incR; i += stride) {
        if (point_in_forward_map<map_t>(i, j, k)) {
          size_t mapIdx = coord_to_rel_coord_map_ijk_to_idx<map_t>(i, j, k);
          retVal[mapIdx] = idx;
          idx += 1;
        }
      }
...
constexpr size_t coord_to_rel_coord_map_ijk_to_idx(int i, int j, int k) {
  ...
  int ak{(k + incR) / stride}, aj{(j + incR) / stride}, ai{(i + incR) / stride};
  return ak + cubeSideInc * (aj + cubeSideInc * ai);
}

/// Size of the backwards map. Map is sparse to make mapping from {i, j, k}
/// easier.
template <class RelCoordsMappingDef>
  ...
  for (int i = 0; i < nPos; ++i) {
    for (int j1 = 0; j1 < NCHILD; ++j1) {
      for (int j2 = 0; j2 < NCHILD; ++j2) {
        ...
        int childRelCoord[3] = {};
        childRelCoord[0] = parentRelCoord[0] * 2 - (j1 / 1) % 2 + (j2 / 1) % 2;
        childRelCoord[1] = parentRelCoord[1] * 2 - (j1 / 2) % 2 + (j2 / 2) % 2;
        childRelCoord[2] = parentRelCoord[2] * 2 - (j1 / 4) % 2 + (j2 / 4) % 2;
...
class M2L_idx_map {
 private:
  ...
  // The mapping storage.
  static constexpr array<int[NCHILD * NCHILD], outerArraySize> s_M2LMap =
