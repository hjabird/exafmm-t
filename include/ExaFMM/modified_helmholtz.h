...
class ModifiedHelmholtzFmm : public Fmm<real_t> {
 public:
  ...
  ModifiedHelmholtzFmm(int p_, int ncrit_, real_t wavek_,
                       std::string filename_ = std::string())
      : Fmm<real_t>(p_, ncrit_, filename_) {
    wavek = wavek_;
    if (this->filename.empty()) {
      this->filename = std::string("modified_helmholtz_") +
   ...
   */
  void potential_P2P(RealVec& src_coord, RealVec& src_value, RealVec& trg_coord,
                     RealVec& trg_value) {
    simdvec zero(real_t(0));
    real_t newton_coef =
        16;  // it comes from Newton's method in simd rsqrt function
   ...
   * @param trg_value Vector of potentials of targets.
   */
  void gradient_P2P(RealVec& src_coord, RealVec& src_value, RealVec& trg_coord,
                    RealVec& trg_value) {
    simdvec zero(real_t(0));
    ...
    simdvec coefg(real_t(1.0 / (4 * PI * newton_coef * newton_coef *
                                newton_coef)));  // gradient: r3
    simdvec k(-wavek / newton_coef);
    int nsrcs = src_coord.size() / 3;
    int ntrgs = trg_coord.size() / 3;
    int t;
