 ...
 *
 ******************************************************************************/
#ifndef INCLUDE_BUILD_NON_ADAPTIVE_TREE_H
#define INCLUDE_BUILD_NON_ADAPTIVE_TREE_H
#include "exafmm_t.h"
#include "fmm_base.h"
#include "hilbert.h"
 ...
 */
template <typename T>
void get_bounds(const Bodies<T>& sources, const Bodies<T>& targets, vec3& x0,
                real_t& r0) {
...
void build_tree(Body<T>* sources, int source_begin, int source_end,
                ...
                Node<T>* node, Nodes<T>& nodes, NodePtrs<T>& leafs,
                NodePtrs<T>& nonleafs, FmmBase<T>& fmm) {
  //! Create a tree node
  ...
  if (node->level == fmm.depth) {
    node->is_leaf = true;
    node->trg_value.resize(node->ntrgs * 4,
                           (T)(0.));  // initialize target result vector
  ...
  sort_bodies<T>(node, sources, source_begin, source_end, source_size,
                 source_offsets);
  sort_bodies<T>(node, targets, target_begin, target_end, target_size,
                 target_offsets);
  ...
  for (int c = 0; c < 8; c++) {
    ...
    cout << "create node at level: " << child[c].level << " octant: " << c
         << " sources range: " << source_offsets[c] << " "
         << source_offsets[c] + source_size[c]
         << " targets range: " << target_offsets[c] << " "
         << target_offsets[c] + target_size[c]
         << " sources address: " << sources << " targets address: " << targets
         << endl;
#endif
 ...
 */
template <typename T>
Nodes<T> build_tree(Bodies<T>& sources, Bodies<T>& targets, NodePtrs<T>& leafs,
                    NodePtrs<T>& nonleafs, FmmBase<T>& fmm) {
  Nodes<T> nodes(1);
  ...
  nodes[0].r = fmm.r0;
  nodes[0].level = 0;
  nodes.reserve((pow(8, fmm.depth + 1) - 1) /
                7);  // reserve for 8^(level+1)/7 nodes
  build_tree(&sources[0], 0, sources.size(), &targets[0], 0, targets.size(),
             &nodes[0], nodes, leafs, nonleafs, fmm);
  return nodes;
}
}  // namespace exafmm_t
#endif  // INCLUDE_BUILD_NON_ADAPTIVE_TREE_H
