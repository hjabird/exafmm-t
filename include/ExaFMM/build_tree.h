 ...
 */
template <typename T>
void sort_bodies(Node<T>* const node, Body<T>* const bodies,
                 Body<T>* const buffer, int begin, int end,
...
template <typename T>
void build_tree(Body<T>* sources, Body<T>* sources_buffer, int source_begin,
                int source_end, Body<T>* targets, Body<T>* targets_buffer,
                int target_begin, int target_end, Node<T>* node,
                Nodes<T>& nodes, NodePtrs<T>& leafs, NodePtrs<T>& nonleafs,
  ...
  for (int c = 0; c < 8; c++) {
    ...
    build_tree(sources_buffer, sources, source_offsets[c],
               ...
               target_offsets[c], target_offsets[c] + target_size[c], &child[c],
               nodes, leafs, nonleafs, fmm, !direction);
  }
}
 ...
 * @param leafkeys Vector of leaf Hilbert keys of each level, only used during
 * 2:1 tree balancing
 *
...
Nodes<T> build_tree(Bodies<T>& sources, Bodies<T>& targets, NodePtrs<T>& leafs,
  ...
  build_tree(&sources[0], &sources_buffer[0], 0, sources.size(), &targets[0],
             &targets_buffer[0], 0, targets.size(), &nodes[0], nodes, leafs,
             nonleafs, fmm);
  int depth = -1;
  for (auto leaf : leafs) {
