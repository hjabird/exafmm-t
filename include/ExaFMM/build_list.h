...
namespace ExaFMM {
	...
	void build_other_list(Node<typename FmmT::potential_t>* node, Nodes<typename FmmT::potential_t>& nodes, const FmmT& fmm,
		const unordered_set<uint64_t>& leaf_keys,
		...
		for (auto i = P2P_set.begin(); i != P2P_set.end();
			i++) {
			if ((*i) != nullptr) {
				curr->P2P_list.push_back(*i);
			}
		}
		for (auto i = P2L_set.begin(); i != P2L_set.end();
			i++) {
			if ((*i) != nullptr) {
				curr->P2L_list.push_back(*i);
			}
		}
		for (auto i = M2P_set.begin(); i != M2P_set.end();
			i++) {
			if ((*i) != nullptr) {
