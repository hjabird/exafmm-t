...
void build_other_list(Node<T>* node, Nodes<T>& nodes, const FmmBase<T>& fmm,
  ...
  for (typename set<Node<T>*>::iterator i = P2P_set.begin(); i != P2P_set.end();
       i++) {
    if ((*i) != nullptr) {
      curr->P2P_list.push_back(*i);
    }
  }
  for (typename set<Node<T>*>::iterator i = P2L_set.begin(); i != P2L_set.end();
       i++) {
    if ((*i) != nullptr) {
      curr->P2L_list.push_back(*i);
    }
  }
  for (typename set<Node<T>*>::iterator i = M2P_set.begin(); i != M2P_set.end();
       i++) {
    if ((*i) != nullptr) {
