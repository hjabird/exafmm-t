 ...
 *
 ******************************************************************************/
#ifndef INCLUDE_EXAFMM_VEC_H_
#define INCLUDE_EXAFMM_VEC_H_
...
class vec {
 ...
 public:
  ...
  vec(const T *v, size_t stride) {
    for (int i = 0; i < N; i++)
      data[i * stride / sizeof(T)] = *(v + stride * i);
  }
  vec(const vec &v) {
  ...
  bool operator!=(const vec &v) { return (!(*this == v)); }
  vec operator&(const vec &v) const { return vec(*this) &= v; }
  vec operator|(const vec &v) const { return vec(*this) |= v; }
  vec operator-() const {
    vec temp;
...
class vec {
 ...
 public:
  __host__ __device__ __forceinline__ vec() {}
  __host__ __device__ __forceinline__ vec(const T &v) {
    Unroll<Ops::Assign<T>, T, N>::loop(data, v);
  }
  __host__ __device__ __forceinline__ vec(const vec &v) {
    Unroll<Ops::Assign<T>, T, N>::loop(data, v);
  }
  __host__ __device__ __forceinline__ vec(const float4 &v) {
    data[0] = v.x;
    data[1] = v.y;
    data[2] = v.z;
    data[3] = v.w;
  }
  __host__ __device__ __forceinline__ vec(const float x, const float y,
  ...
  __host__ __device__ __forceinline__ ~vec() {}
  __host__ __device__ __forceinline__ const vec &operator=(const T v) {
    Unroll<Ops::Assign<T>, T, N>::loop(data, v);
    return *this;
  }
  __host__ __device__ __forceinline__ const vec &operator+=(const T v) {
  ...
  __host__ __device__ __forceinline__ const vec &operator=(const vec &v) {
    Unroll<Ops::Assign<T>, T, N>::loop(data, v);
    return *this;
  }
  __host__ __device__ __forceinline__ const vec &operator+=(const vec &v) {
  ...
  __host__ __device__ __forceinline__ vec operator+(const T v) const {
    return vec(*this) += v;
  }
  __host__ __device__ __forceinline__ vec operator-(const T v) const {
    return vec(*this) -= v;
  ...
  __host__ __device__ __forceinline__ vec operator+(const vec &v) const {
    return vec(*this) += v;
  }
  __host__ __device__ __forceinline__ vec operator-(const vec &v) const {
    return vec(*this) -= v;
  ...
  __host__ __device__ __forceinline__ vec operator-() const {
    vec temp;
    Unroll<Ops::Negate<T>, T, N>::loop(temp, data);
    return temp;
  }
  __host__ __device__ __forceinline__ T &operator[](int i) { return data[i]; }
  ...
  __host__ __device__ __forceinline__ friend T min(const vec &v) {
    T temp;
    for (int i = 0; i < N; i++) temp = temp < v[i] ? temp : v[i];
    return temp;
  }
  __host__ __device__ __forceinline__ friend T max(const vec &v) {
    T temp;
    for (int i = 0; i < N; i++) temp = temp > v[i] ? temp : v[i];
    return temp;
  }
  ...
  __host__ __device__ __forceinline__ friend std::ostream &operator<<(
      std::ostream &s, const vec &v) {
    for (int i = 0; i < N; i++) s << v[i] << ' ';
    return s;
  }
  __host__ __device__ __forceinline__ friend T sum(const vec &v) {
    return Unroll<Ops::Add<T>, T, N>::reduce(v);
  }
  __host__ __device__ __forceinline__ friend T norm(const vec &v) {
    return sum(v * v);
  }
  __host__ __device__ __forceinline__ friend vec min(const vec &v,
                                                     const vec &w) {
    vec temp;
    for (int i = 0; i < N; i++) temp[i] = v[i] < w[i] ? v[i] : w[i];
    return temp;
  }
  __host__ __device__ __forceinline__ friend vec max(const vec &v,
                                                     const vec &w) {
    vec temp;
    for (int i = 0; i < N; i++) temp[i] = v[i] > w[i] ? v[i] : w[i];
    return temp;
  ...
  __device__ __forceinline__ friend vec abs(const vec &v) {
    vec temp;
    Unroll<Ops::Abs<T>, T, N>::loop(temp, v);
    return temp;
  }
  __device__ __forceinline__ friend vec rsqrt(const vec &v) {
    vec temp;
    Unroll<Ops::Rsqrt<T>, T, N>::loop(temp, v);
    return temp;
  }
  __host__ __device__ __forceinline__ friend vec sin(const vec &v) {
    vec temp;
    Unroll<Ops::Sin<T>, T, N>::loop(temp, v);
    return temp;
  }
  __host__ __device__ __forceinline__ friend vec cos(const vec &v) {
    vec temp;
    Unroll<Ops::Cos<T>, T, N>::loop(temp, v);
    return temp;
  }
  __host__ __device__ __forceinline__ friend void sincos(vec &s, vec &c,
                                                         const vec &v) {
    Unroll<Ops::SinCos<T>, T, N>::loop(s, c, v);
  }
  __host__ __device__ __forceinline__ friend vec exp(const vec &v) {
    vec temp;
    Unroll<Ops::Exp<T>, T, N>::loop(temp, v);
    return temp;
  }
  __host__ __device__ __forceinline__ friend int wrap(vec &v, const T &w) {
    int iw = 0;
    for (int i = 0; i < N; i++) {
      if (v[i] < -w / 2) {
        v[i] += w;
  ...
  __host__ __device__ __forceinline__ friend void unwrap(vec &v, const T &w,
                                                         const int &iw) {
    for (int i = 0; i < N; i++) {
...
class vec<16, float> {
 ...
 public:
  vec() {}
  vec(const float v) { data = _mm512_set1_ps(v); }
  vec(const __m512 v) { data = v; }
  vec(const vec &v) { data = v.data; }
  vec(const float a, const float b, const float c, const float d, const float e,
      const float f, const float g, const float h, const float i, const float j,
  ...
  vec(const float *a, const int size) {
    ...
    data = _mm512_setr_ps(
        *a, *(a + 1 * offset), *(a + 2 * offset), *(a + 3 * offset),
        *(a + 4 * offset), *(a + 5 * offset), *(a + 6 * offset),
        *(a + 7 * offset), *(a + 8 * offset), *(a + 9 * offset),
        *(a + 10 * offset), *(a + 11 * offset), *(a + 12 * offset),
        *(a + 13 * offset), *(a + 14 * offset), *(a + 15 * offset));
  }
  ~vec() {}
  ...
  vec operator+(const vec &v) const { return vec(_mm512_add_ps(data, v.data)); }
  vec operator-(const vec &v) const { return vec(_mm512_sub_ps(data, v.data)); }
  vec operator*(const vec &v) const { return vec(_mm512_mul_ps(data, v.data)); }
  vec operator/(const vec &v) const { return vec(_mm512_div_ps(data, v.data)); }
  ...
  friend vec sin(const vec &v) { return vec(_mm512_sin_ps(v.data)); }
  friend vec cos(const vec &v) { return vec(_mm512_cos_ps(v.data)); }
  friend vec exp(const vec &v) { return vec(_mm512_exp_ps(v.data)); }
#else
  friend vec sin(const vec &v) {
    vec temp = _mm512_setr_ps(
        std::sin(v[0]), std::sin(v[1]), std::sin(v[2]), std::sin(v[3]),
  ...
  friend vec cos(const vec &v) {
    vec temp = _mm512_setr_ps(
        std::cos(v[0]), std::cos(v[1]), std::cos(v[2]), std::cos(v[3]),
  ...
  friend vec exp(const vec &v) {
    vec temp = _mm512_setr_ps(
        std::exp(v[0]), std::exp(v[1]), std::exp(v[2]), std::exp(v[3]),
...
class vec<8, double> {
 ...
 public:
  vec() {}
  vec(const double v) { data = _mm512_set1_pd(v); }
  vec(const __m512d v) { data = v; }
  vec(const vec &v) { data = v.data; }
  ...
  vec(const double *a) {
    data = _mm512_setr_pd(*a, *(a + 1), *(a + 2), *(a + 3), *(a + 4), *(a + 5),
                          *(a + 6), *(a + 7));
  }
  vec(const double *a, const int size) {
    int offset = size / (int)sizeof(double);
    data =
        _mm512_setr_pd(*a, *(a + 1 * offset), *(a + 2 * offset),
                       *(a + 3 * offset), *(a + 4 * offset), *(a + 5 * offset),
                       *(a + 6 * offset), *(a + 7 * offset));
  }
  ~vec() {}
  ...
  vec operator+(const vec &v) const { return vec(_mm512_add_pd(data, v.data)); }
  vec operator-(const vec &v) const { return vec(_mm512_sub_pd(data, v.data)); }
  vec operator*(const vec &v) const { return vec(_mm512_mul_pd(data, v.data)); }
  vec operator/(const vec &v) const { return vec(_mm512_div_pd(data, v.data)); }
  ...
  const double &operator[](int i) const { return array[i]; }
  friend std::ostream &operator<<(std::ostream &s, const vec &v) {
...
#ifdef __INTEL_COMPILER
  friend vec sin(const vec &v) { return vec(_mm512_sin_pd(v.data)); }
  friend vec cos(const vec &v) { return vec(_mm512_cos_pd(v.data)); }
  friend vec exp(const vec &v) { return vec(_mm512_exp_pd(v.data)); }
#else
  friend vec sin(const vec &v) {
    vec temp = _mm512_setr_pd(std::sin(v[0]), std::sin(v[1]), std::sin(v[2]),
                              std::sin(v[3]), std::sin(v[4]), std::sin(v[5]),
                              std::sin(v[6]), std::sin(v[7]));
    return temp;
  }
  friend vec cos(const vec &v) {
    vec temp = _mm512_setr_pd(std::cos(v[0]), std::cos(v[1]), std::cos(v[2]),
                              std::cos(v[3]), std::cos(v[4]), std::cos(v[5]),
                              std::cos(v[6]), std::cos(v[7]));
    return temp;
  }
  friend vec exp(const vec &v) {
    vec temp = _mm512_setr_pd(std::exp(v[0]), std::exp(v[1]), std::exp(v[2]),
                              std::exp(v[3]), std::exp(v[4]), std::exp(v[5]),
                              std::exp(v[6]), std::exp(v[7]));
    return temp;
  }
#endif
};
...
class vec<8, float> {
 ...
 public:
  vec() {}
  vec(const float v) { data = _mm256_set1_ps(v); }
  vec(const __m256 v) { data = v; }
  vec(const vec &v) { data = v.data; }
  vec(const float a, const float b, const float c, const float d, const float e,
      const float f, const float g, const float h) {
    data = _mm256_setr_ps(a, b, c, d, e, f, g, h);
  }
  vec(const float *a, const int size) {
    int offset = size / (int)sizeof(float);
    data =
        _mm256_setr_ps(*a, *(a + 1 * offset), *(a + 2 * offset),
                       *(a + 3 * offset), *(a + 4 * offset), *(a + 5 * offset),
                       *(a + 6 * offset), *(a + 7 * offset));
  }
  ~vec() {}
  ...
  vec operator+(const vec &v) const { return vec(_mm256_add_ps(data, v.data)); }
  vec operator-(const vec &v) const { return vec(_mm256_sub_ps(data, v.data)); }
  vec operator*(const vec &v) const { return vec(_mm256_mul_ps(data, v.data)); }
  vec operator/(const vec &v) const { return vec(_mm256_div_ps(data, v.data)); }
...
#ifdef __INTEL_COMPILER
  friend vec sin(const vec &v) { return vec(_mm256_sin_ps(v.data)); }
  friend vec cos(const vec &v) { return vec(_mm256_cos_ps(v.data)); }
  friend vec exp(const vec &v) { return vec(_mm256_exp_ps(v.data)); }
#else
  friend vec sin(const vec &v) {
    vec temp = _mm256_setr_ps(std::sin(v[0]), std::sin(v[1]), std::sin(v[2]),
                              std::sin(v[3]), std::sin(v[4]), std::sin(v[5]),
                              std::sin(v[6]), std::sin(v[7]));
    return temp;
  }
  friend vec cos(const vec &v) {
    vec temp = _mm256_setr_ps(std::cos(v[0]), std::cos(v[1]), std::cos(v[2]),
                              std::cos(v[3]), std::cos(v[4]), std::cos(v[5]),
                              std::cos(v[6]), std::cos(v[7]));
    return temp;
  }
  friend vec exp(const vec &v) {
    vec temp = _mm256_setr_ps(std::exp(v[0]), std::exp(v[1]), std::exp(v[2]),
                              std::exp(v[3]), std::exp(v[4]), std::exp(v[5]),
                              std::exp(v[6]), std::exp(v[7]));
    return temp;
  }
...
class vec<4, double> {
 ...
 public:
  vec() {}
  vec(const double v) { data = _mm256_set1_pd(v); }
  vec(const __m256d v) { data = v; }
  vec(const vec &v) { data = v.data; }
  vec(const double a, const double b, const double c, const double d) {
  ...
  vec(const double *a, const int size) {
    int offset = size / (int)sizeof(double);
    data = _mm256_setr_pd(*a, *(a + 1 * offset), *(a + 2 * offset),
                          *(a + 3 * offset));
  }
  ~vec() {}
  const vec &operator=(const double v) {
  ...
  vec operator+(const vec &v) const { return vec(_mm256_add_pd(data, v.data)); }
  vec operator-(const vec &v) const { return vec(_mm256_sub_pd(data, v.data)); }
  vec operator*(const vec &v) const { return vec(_mm256_mul_pd(data, v.data)); }
  vec operator/(const vec &v) const { return vec(_mm256_div_pd(data, v.data)); }
  vec operator>(const vec &v) const {
...
#ifdef __INTEL_COMPILER
  friend vec sin(const vec &v) { return vec(_mm256_sin_pd(v.data)); }
  friend vec cos(const vec &v) { return vec(_mm256_cos_pd(v.data)); }
  friend vec exp(const vec &v) { return vec(_mm256_exp_pd(v.data)); }
#else
  friend vec sin(const vec &v) {
    vec temp = _mm256_setr_pd(std::sin(v[0]), std::sin(v[1]), std::sin(v[2]),
                              std::sin(v[3]));
    return temp;
  }
  friend vec cos(const vec &v) {
    vec temp = _mm256_setr_pd(std::cos(v[0]), std::cos(v[1]), std::cos(v[2]),
                              std::cos(v[3]));
    return temp;
  }
  friend vec exp(const vec &v) {
    vec temp = _mm256_setr_pd(std::exp(v[0]), std::exp(v[1]), std::exp(v[2]),
                              std::exp(v[3]));
    return temp;
  }
#endif
...
class vec<4, float> {
 ...
 public:
  vec() {}
  vec(const float v) { data = _mm_set1_ps(v); }
  vec(const __m128 v) { data = v; }
  vec(const vec &v) { data = v.data; }
  ...
  vec(const float *a, const int size) {
    int offset = size / (int)sizeof(float);
    data = _mm_setr_ps(*a, *(a + 1 * offset), *(a + 2 * offset),
                       *(a + 3 * offset));
  }
  ~vec() {}
  const vec &operator=(const float v) {
  ...
  vec operator+(const vec &v) const { return vec(_mm_add_ps(data, v.data)); }
  vec operator-(const vec &v) const { return vec(_mm_sub_ps(data, v.data)); }
  vec operator*(const vec &v) const { return vec(_mm_mul_ps(data, v.data)); }
  vec operator/(const vec &v) const { return vec(_mm_div_ps(data, v.data)); }
  vec operator>(const vec &v) const { return vec(_mm_cmpgt_ps(data, v.data)); }
  vec operator<(const vec &v) const { return vec(_mm_cmplt_ps(data, v.data)); }
  vec operator-() const { return vec(_mm_sub_ps(_mm_setzero_ps(), data)); }
  float &operator[](int i) { return array[i]; }
  const float &operator[](int i) const { return array[i]; }
  friend std::ostream &operator<<(std::ostream &s, const vec &v) {
    for (int i = 0; i < 4; i++) s << v[i] << ' ';
...
#ifdef __INTEL_COMPILER
  friend vec sin(const vec &v) { return vec(_mm_sin_ps(v.data)); }
  friend vec cos(const vec &v) { return vec(_mm_cos_ps(v.data)); }
  friend vec exp(const vec &v) { return vec(_mm_exp_ps(v.data)); }
#else
  friend vec sin(const vec &v) {
    vec temp = _mm_setr_ps(std::sin(v[0]), std::sin(v[1]), std::sin(v[2]),
                           std::sin(v[3]));
    return temp;
  }
  friend vec cos(const vec &v) {
    vec temp = _mm_setr_ps(std::cos(v[0]), std::cos(v[1]), std::cos(v[2]),
                           std::cos(v[3]));
    return temp;
  }
  friend vec exp(const vec &v) {
    vec temp = _mm_setr_ps(std::exp(v[0]), std::exp(v[1]), std::exp(v[2]),
                           std::exp(v[3]));
    return temp;
  }
#endif
...
class vec<2, double> {
 ...
 public:
  vec() {}
  vec(const double v) { data = _mm_set1_pd(v); }
  vec(const __m128d v) { data = v; }
  vec(const vec &v) { data = v.data; }
  vec(const double a, const double b) { data = _mm_setr_pd(a, b); }
  vec(const double *a, const int size) {
    int offset = size / (int)sizeof(double);
  ...
  vec operator+(const vec &v) const { return vec(_mm_add_pd(data, v.data)); }
  vec operator-(const vec &v) const { return vec(_mm_sub_pd(data, v.data)); }
  vec operator*(const vec &v) const { return vec(_mm_mul_pd(data, v.data)); }
  vec operator/(const vec &v) const { return vec(_mm_div_pd(data, v.data)); }
  vec operator>(const vec &v) const { return vec(_mm_cmpgt_pd(data, v.data)); }
  vec operator<(const vec &v) const { return vec(_mm_cmplt_pd(data, v.data)); }
  vec operator-() const { return vec(_mm_sub_pd(_mm_setzero_pd(), data)); }
  double &operator[](int i) { return array[i]; }
  const double &operator[](int i) const { return array[i]; }
  friend std::ostream &operator<<(std::ostream &s, const vec &v) {
...
#ifdef __INTEL_COMPILER
  friend vec sin(const vec &v) { return vec(_mm_sin_pd(v.data)); }
  friend vec cos(const vec &v) { return vec(_mm_cos_pd(v.data)); }
  friend vec exp(const vec &v) { return vec(_mm_exp_pd(v.data)); }
#else
  friend vec sin(const vec &v) {
