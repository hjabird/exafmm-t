...
class FmmScaleInvariant : public FmmBase<T> {
 ...
 public:
  ...
  std::vector<T>
      matrix_UC2E_U;  //!< First component of the pseudo-inverse of upward check
                      //!< to upward equivalent kernel matrix.
  std::vector<T>
      matrix_UC2E_V;  //!< Second component of the pseudo-inverse of upward
                      //!< check to upward equivalent kernel matrix.
  std::vector<T>
      matrix_DC2E_U;  //!< First component of the pseudo-inverse of downward
                      //!< check to downward equivalent kernel matrix.
  std::vector<T>
      matrix_DC2E_V;  //!< Second component of the pseudo-inverse of downward
                      //!< check to downward equivalent kernel matrix.
  std::vector<std::vector<T>>
      matrix_M2M;  //!< The pseudo-inverse of M2M kernel matrix.
  std::vector<std::vector<T>>
      matrix_L2L;  //!< The pseudo-inverse of L2L kernel matrix.
  std::vector<AlignedVec>
      matrix_M2L;  //!< The pseudo-inverse of M2L kernel matrix.
  ...
  FmmScaleInvariant(int p_, int ncrit_, std::string filename_ = std::string())
      : FmmBase<T>(p_, ncrit_, filename_) {}

  /* precomputation */
  //! Setup the sizes of precomputation matrices
  void initialize_matrix() {
    size_t size = this->nfreq * 2 * NCHILD *
                  NCHILD;  // size of each M2L precomputation matrix
    ...
    matrix_M2M.resize(REL_COORD[M2M_Type].size(),
                      std::vector<T>(nsurf_ * nsurf_));
    matrix_L2L.resize(REL_COORD[L2L_Type].size(),
                      std::vector<T>(nsurf_ * nsurf_));
...
#pragma omp parallel for
    for (int i = 0; i < npos; i++) {
      ...
      gemm(nsurf_, nsurf_, nsurf_, &matrix_UC2E_U[0], &matrix_pc2ce[0],
           &buffer[0]);
      gemm(nsurf_, nsurf_, nsurf_, &matrix_UC2E_V[0], &buffer[0],
           &(matrix_M2M[i][0]));
      // L2L
      matrix_pc2ce = transpose(matrix_pc2ce, nsurf_, nsurf_);
      gemm(nsurf_, nsurf_, nsurf_, &matrix_pc2ce[0], &matrix_DC2E_V[0],
           &buffer[0]);
      gemm(nsurf_, nsurf_, nsurf_, &buffer[0], &matrix_DC2E_U[0],
           &(matrix_L2L[i][0]));
    }
  }
  ...
  void load_matrix() {
    ...
    size_t file_size = (2 * REL_COORD[M2M_Type].size() + 4) * this->nsurf *
                           this->nsurf * sizeof(T) +
    ...
    if (file.good()) {
      ...
      if (size_t(file.tellg()) == file_size) {  // if file size is correct
        ...
        if (this->r0 == r0_) {  // if radius match
          size_t size = this->nsurf * this->nsurf;
          // UC2E, DC2E
          file.read(reinterpret_cast<char*>(&matrix_UC2E_U[0]),
                    size * sizeof(T));
          file.read(reinterpret_cast<char*>(&matrix_UC2E_V[0]),
                    size * sizeof(T));
          file.read(reinterpret_cast<char*>(&matrix_DC2E_U[0]),
                    size * sizeof(T));
          file.read(reinterpret_cast<char*>(&matrix_DC2E_V[0]),
                    size * sizeof(T));
          // M2M, L2L
          for (auto& vec : matrix_M2M) {
          ...
          // M2L
          for (auto& vec : matrix_M2L) {
            file.read(reinterpret_cast<char*>(&vec[0]),
                      size_M2L * sizeof(real_t));
          }
...
#pragma omp parallel for
    for (size_t i = 0; i < leafs.size(); i++) {
      ...
      real_t scale =
          pow(0.5, level);  // scaling factor of UC2UE precomputation matrix
      ...
      for (int k = 0; k < nsurf_; k++) leaf->up_equiv[k] = scale * equiv[k];
    }
  }
...
#pragma omp parallel for
    for (size_t i = 0; i < leafs.size(); i++) {
      ...
      // scale the check-to-equivalent conversion (precomputation)
      for (int k = 0; k < nsurf_; k++) leaf->dn_equiv[k] = scale * equiv[k];
      // calculate targets' potential & gradient induced by downward equivalent
      // charge
...
#pragma omp taskwait
    // evaluate parent's upward equivalent charge from child's upward equivalent
    // charge
    for (int octant = 0; octant < 8; octant++) {
      if (node->children[octant]) {
        Node<T>* child = node->children[octant];
        std::vector<T> buffer(nsurf_);
        gemv(nsurf_, nsurf_, &(matrix_M2M[octant][0]), &child->up_equiv[0],
             &buffer[0]);
        for (int k = 0; k < nsurf_; k++) {
  ...
  void L2L(Node<T>* node) {
    int& nsurf_ = this->nsurf;
    if (node->is_leaf) return;
    // evaluate child's downward check potential from parent's downward check
    // potential
    for (int octant = 0; octant < 8; octant++) {
      if (node->children[octant]) {
        Node<T>* child = node->children[octant];
        std::vector<T> buffer(nsurf_);
        gemv(nsurf_, nsurf_, &(matrix_L2L[octant][0]), &node->dn_equiv[0],
             &buffer[0]);
...
#pragma omp parallel for
      for (int k = 0; k < this->nfreq; k++) {
        for (size_t ipos = 0; ipos < npos; ipos++) {
          ...
          real_t* M = &matrix_M2L[ipos][k * 2 * NCHILD *
                                        NCHILD];  // k-th freq's (row) offset in
                                                  // matrix_M2L[ipos]
    ...
    // add flop
    add_flop((long long)(8 * 8 * 8) * (interaction_offset_f.size() / 2) *
             this->nfreq);
  }

  void fft_up_equiv(std::vector<size_t>& fft_offset, RealVec& all_up_equiv,
                    AlignedVec& fft_in) {}
  ...
  void M2L(Nodes<T>& nodes) {
    ...
    all_up_equiv.reserve(
        nnodes *
        nsurf_);  // use reserve() to avoid the overhead of calling constructor
    all_dn_equiv.reserve(
        nnodes * nsurf_);  // use pointer instead of iterator to access elements
...
#pragma omp parallel for collapse(2)
    ...
    hadamard_product(m2ldata.interaction_count_offset,
                     m2ldata.interaction_offset_f, fft_in, fft_out);
    ifft_dn_check(m2ldata.ifft_offset, m2ldata.ifft_scale, fft_out,
                  all_dn_equiv);
...
#pragma omp parallel for
  for (long long int i = 0; i < REL_COORD[M2L_Helper_Type].size(); ++i) {
    ...
    for (int d = 0; d < 3; d++) {
      coord[d] =
          REL_COORD[M2L_Helper_Type][i][d] * this->r0 / 0.5;  // relative coords
    }
    RealVec conv_coord =
        convolution_grid(this->p, this->r0, 0, coord);  // convolution grid
...
#pragma omp parallel for
  for (long long int node_idx = 0; node_idx < fft_offset.size(); node_idx++) {
    ...
    // since it's larger than what we need,  we can use fft_out as fftw_in
    // buffer here
    real_t* up_equiv_f =
        &fft_in[fft_size * node_idx];  // offset ptr of node_idx in fft_in
                                       // vector, size=fft_size
    std::memset(up_equiv_f, 0,
                fft_size * sizeof(real_t));  // initialize fft_in to 0
    for (int k = 0; k < nsurf_; k++) {
      size_t idx = map[k];
...
template <>
void FmmScaleInvariant<real_t>::ifft_dn_check(std::vector<size_t>& ifft_offset,
                                              RealVec& ifft_scal,
                                              AlignedVec& fft_out,
...
#pragma omp parallel for
  for (long long int node_idx = 0; node_idx < ifft_offset.size(); node_idx++) {
    ...
    for (int k = 0; k < nsurf_; k++) {
      ...
      for (int j = 0; j < NCHILD; j++)
        dn_equiv[nsurf_ * j + k] +=
            buffer1[idx + j * nconv_] * ifft_scal[node_idx];
    }
  }
  fft_destroy_plan(plan);
}
