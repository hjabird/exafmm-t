...
void zgesvd_(char* JOBU, char* JOBVT, int* M, int* N, complex<double>* A,
             ...
             complex<double>* VT, int* LDVT, complex<double>* WORK, int* LWORK,
...
void svd(int m, int n, real_t* A, real_t* S, real_t* U, real_t* VT) {
  ...
  RealVec WORK(LWORK);
#if FLOAT
  sgesvd_(&JOBU, &JOBVT, &n, &m, A, &n, &tS[0], VT, &n, U, &k, &WORK[0], &LWORK,
          &INFO);
#else
  dgesvd_(&JOBU, &JOBVT, &n, &m, A, &n, &tS[0], VT, &n, U, &k, &WORK[0], &LWORK,
          &INFO);
#endif
...
void svd(int m, int n, complex_t* A, real_t* S, complex_t* U, complex_t* VT) {
  ...
  RealVec RWORK(5 * k);
#if FLOAT
  cgesvd_(&JOBU, &JOBVT, &n, &m, A, &n, &tS[0], VT, &n, U, &k, &WORK[0], &LWORK,
          &RWORK[0], &INFO);
#else
  zgesvd_(&JOBU, &JOBVT, &n, &m, A, &n, &tS[0], VT, &n, U, &k, &WORK[0], &LWORK,
          &RWORK[0], &INFO);
#endif
  // copy singular values from 1d layout (tS) to 2d layout (S)
  for (int i = 0; i < k; i++) {
