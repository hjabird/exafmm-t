...
//
// void cgemv_(char* trans, int* m, int* n, complex<float>* alpha,
...
//  char JOBU = 'S', JOBVT = 'S';
//  int INFO;
//  int LWORK = std::max(3 * std::min(m, n) + std::max(m, n), 5 * std::min(m,
//  n)); LWORK = std::max(LWORK, 1); int k = std::min(m, n); RealVec tS(k, 0.);
//  RealVec WORK(LWORK);
//#if FLOAT
//  sgesvd_(&JOBU, &JOBVT, &n, &m, A, &n, &tS[0], VT, &n, U, &k, &WORK[0],
//  &LWORK,
//          &INFO);
//#else
//  dgesvd_(&JOBU, &JOBVT, &n, &m, A, &n, &tS[0], VT, &n, U, &k, &WORK[0],
//  &LWORK,
//          &INFO);
//#endif
...
//  RealVec RWORK(5 * k);
//#if FLOAT
//  cgesvd_(&JOBU, &JOBVT, &n, &m, A, &n, &tS[0], VT, &n, U, &k, &WORK[0],
//  &LWORK,
//          &RWORK[0], &INFO);
//#else
//  zgesvd_(&JOBU, &JOBVT, &n, &m, A, &n, &tS[0], VT, &n, U, &k, &WORK[0],
//  &LWORK,
//          &RWORK[0], &INFO);
//#endif
//  // copy singular values from 1d layout (tS) to 2d layout (S)
